...
<div class="max-w-7xl mx-auto px-4">
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-10">
        <form action="" method="GET" class="flex-1 relative">
            ...
            <input type="text" name="search" id="adminSearchInput" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search students by name, email or username..." class="w-full pl-12 pr-4 py-4 rounded-2xl border border-gray-100 bg-white focus:outline-none focus:ring-4 focus:ring-navy/5 focus:border-navy transition-all shadow-sm text-sm">
        </form>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-navy text-white">
                        <th class="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em]">Full Name</th>
                        <th class="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em]">Contact Info</th>
                        <th class="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em]">Joined Date</th>
                        <th class="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em]">Courses</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    <?php if (empty($students)): ?>
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-gray-400 font-bold">No students found.</td>
                        </tr>
                    <?php else: foreach ($students as $student): ?>
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-6 py-5 flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-bold text-navy"><?php echo htmlspecialchars($student['name']); ?></p>
                                    <p class="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">@<?php echo htmlspecialchars($student['username']); ?></p>
                                </div>
                            </td>
                            <td class="px-6 py-5">
                                <p class="text-xs font-bold text-gray-600"><?php echo htmlspecialchars($student['email']); ?></p>
                                <p class="text-[10px] text-blue-500 font-bold"><?php echo htmlspecialchars($student['whatsapp_number']); ?></p>
                            </td>
                            <td class="px-6 py-5 text-sm font-bold text-navy">
                                <?php echo date('M d, Y', strtotime($student['created_at'])); ?>
                            </td>
                            <td class="px-6 py-5">
                                <?php if ($student['enrollment_count'] > 0): ?>
                                    <span class="bg-green-50 text-green-600 px-3 py-1 rounded-full text-sm font-black border border-green-100 shadow-sm">
                                        <?php echo $student['enrollment_count']; ?>
                                    </span>
                                <?php else: ?>
                                    <span class="text-gray-300 font-bold text-xs">0</span>
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-5">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-2xl shadow-black/20 overflow-hidden transform transition-all scale-95 opacity-0 duration-300" id="modalContainer">
            ...
            <div class="relative h-32 bg-navy flex items-end px-8 pb-4">
                ...
                <div class="flex items-center gap-4 translate-y-8">
                    <div id="studentInitials" class="w-20 h-20 rounded-2xl bg-white text-navy flex items-center justify-center text-2xl font-black shadow-xl">
                        --
                    </div>
                </div>
            </div>
            ...
            <div class="pt-12 px-8 pb-8">
                <div class="flex justify-between items-start mb-8">
                    <div>
                        <h2 id="studentName" class="text-2xl font-black text-navy">Loading...</h2>
                        <p id="studentEmail" class="text-sm font-bold text-gray-400">--</p>
                    </div>
                    <div class="text-right">
                        <p class="text-[10px] font-black uppercase text-gray-300 tracking-[0.2em] mb-1">Joined Date</p>
                        <p id="studentJoined" class="text-xs font-bold text-navy">--</p>
                    </div>
                </div>

                <div class="grid grid-cols-2 gap-4 mb-8">
                    <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                        <p class="text-[10px] font-black uppercase text-gray-400 tracking-widest mb-1">WhatsApp</p>
                        <p id="studentPhone" class="text-sm font-bold text-navy">--</p>
                    </div>
                    <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                        <p class="text-[10px] font-black uppercase text-gray-400 tracking-widest mb-1">Username</p>
                        <p id="studentUser" class="text-sm font-bold text-navy">--</p>
                    </div>
                </div>

                <h3 class="font-black text-navy uppercase text-xs tracking-[0.2em] mb-4 border-b border-gray-100 pb-2">Registered Courses</h3>
...
function viewStudent(id) {
    ...
    // Reset UI
    document.getElementById('courseList').innerHTML = '<div class="text-center py-4 text-gray-400 text-sm">Loading courses...</div>';
        ...
        .then(data => {
            if (data.success) {
                ...
                if (data.courses.length === 0) {
                    html = '<div class="text-center py-6 border-2 border-dashed border-gray-100 rounded-2xl text-gray-300 font-bold">No courses registered yet.</div>';
                } else {
                        ...
                        html += `
                            <div class="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl hover:border-navy/20 transition-all hover:shadow-md group">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-bold text-navy">${c.course_title}</p>
                                        <p class="text-[9px] font-black uppercase text-gray-400">Applied: ${new Date(c.applied_date).toLocaleDateString()}</p>
                                    </div>
                                </div>
                                <div class="flex flex-col items-end gap-1">
                                    <span class="text-[10px] font-black text-navy">$${c.price}</span>
